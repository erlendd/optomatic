...
config['max_evals'] = 9
config['features'] = '111'
config['project_name'] = "example_multiexperiment"
...
for clf_name in user.clfs:
    ...
    # multi-experiment (batch) mode: all experiments go into one yaml file:
...
ywrite(config, 'experiments.yaml')
    
print "To run these jobs run ./worker.py --conf experiments.yaml"
