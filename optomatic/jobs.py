...
class JobsDB:
    ...
    def __init__(self, project_name, experiment_name, host='localhost', port=27017, param_space=None, param_types=None):
    ...
    def get_next_job_from_queue(self, owner=None):
        _owner = self.owner
        if owner is not None: _owner = owner
        result = self.collection.find_and_modify( {"status": self.STATUS_QUEUED}, 
    ...
    def report_job_completion(self, jobID, loss, aux_data=None):
        result = self.collection.update_one( {"status": self.STATUS_RUNNING, "_id": jobID},
    ...
    def get_best_run(self, count=1):
        ...
        result = self.collection.find( {'status': self.STATUS_COMPLETED} ).sort( {'loss': 1} ).limit(count)
        return result
    ...
    def get_orphaned_jobs(self, time_until_old=1800, owner=None):
        ...
        needle = {'status': self.STATUS_RUNNING, 'refresh_time': {'$lt': time_until_old}}
    ...
    def setup_value_encoder(self, param_space, param_types=None):
        ...
        for param_name in param_space:
            ...
            if param_types is not None: 
                ...
                self.param_value_encoder[param_name] = False # False, until we find reason otherwise...
                for param_value in param_space[param_name]: # ...check for that reason now...
                    if isinstance( param_value, unicode ) or isinstance( param_value, str ):
                        self.param_value_encoder[param_name] = LabelEncoder() 
        ...
        if encode_labels:
            for pname in self.param_value_encoder:
                if self.param_value_encoder[pname]:
                    ...
                    param_values[pname] = self.param_value_encoder[pname].transform( param_values[pname] )
    ...
    def print_job_stats(self):
        ...
        print ' Job Statistics for {} in {}'.format(self.experiment_name, self.project_name)
        ...
        print '   n_remaining  =  {}'.format( (all_jobs.count() - completed_jobs.count()) )
